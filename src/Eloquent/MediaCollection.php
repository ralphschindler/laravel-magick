 ...
 * @mixin Collection
 */
class MediaCollection implements Arrayable, ArrayAccess, Countable, IteratorAggregate, JsonSerializable, Jsonable
{
    use ForwardsCalls;

    /** @var Collection|AbstractMedia[] */
    protected $images;
    ...
    public function __construct(protected AbstractMedia $prototype)
    {
    ...
    {
        $images = $this->images->map(function (AbstractMedia $image) {
    ...
    {
        $this->images->each(function (AbstractMedia $image) use ($replacements, $model) {

            if ($image->index === null) {
                $image->setIndex($this->autoincrement++);
            }

            $image->updatePath($replacements, $model);
        });
    }
    ...
    {
        $this->images = $this->images->filter(function (AbstractMedia $image) {
            $this->deletedImages[] = $image;
            $image->remove();
    ...
    public function flush()
    {
        ...
        $this->images->each(function (AbstractMedia $image) {
            $image->flush();
        });
    }
