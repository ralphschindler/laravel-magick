  ...
  <default-field :field="field" :errors="errors" :full-width-content="true">
    <template slot="field">
      <div class="bg-white rounded-lg">
        <draggable v-model="images" group="image-group" v-on:start="drag=true" v-on:end="drag=false" :class="`flex flex-wrap mb-2 laravel-magick-${this.resourceName}`">
          <div v-for="(image, index) in images" :class="`pl-1 pr-1 border border-70 flex items-end m-1 laravel-magick-image-${(index + 1)}`">
          ...
          <div v-show="!isReadonly"  v-if="(isCollection == false && images.length == 0) || isCollection" slot="footer" class="flex items-center pl-1 pr-1 m-1 border border-70">
            <div class="content-center px-6 py-4">
              <input
                ...
                type="file"
                v-bind:id="`magick-` + this.field.name + `-add-image`"
                name="name"
...
<script>
  ...
  export default {
    ...
    computed: {
      images: {
        get () {
          return this.$store.getters[`magick/${this.field.attribute}/getImages`]
        },
        set (value) {
          this.$store.commit(`magick/${this.field.attribute}/updateImages`, value)
        }
      },

      isCollection () {
        return this.$store.getters[`magick/${this.field.attribute}/getIsCollection`]
      }
    },
    ...
    methods: {
      ...
      setInitialValue () {
        ...
        let images = (isCollection ? this.field.value.images : (this.field.value ? [this.field.value] : []))
          .map((image, i) => {
            return {
              inputId: 'magick-' + this.field.attribute.replace('_', '-') + '-' + i,
        ...
        this.$store.commit(`magick/${this.field.attribute}/initialize`, { fieldName: this.field.attribute, isCollection, images })
      },

      addImage (event, metadata = {}) {
        this.$store.dispatch(`magick/${this.field.attribute}/addImageFromFile`, {
          file: event.target.files[0]
        })
      },

      removeImage (image) {
        this.$store.dispatch(`magick/${this.field.attribute}/removeImage`, image)
      },
    ...
    created () {
      this.$store.registerModule(`magick/${this.field.attribute}`, store)
    },

    destroyed () {
      this.$store.unregisterModule(`magick/${this.field.attribute}`)
    }
  }
</script>
